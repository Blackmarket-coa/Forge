...
//! `code_sha256` → `code_blob_sha256` when an entry blob exists). No archive
//! format is produced for `manifest_plugin` (there are no code bytes);
//! `.tgz` bundling is deferred until `code_plugin` publishing is in scope.
...
pub struct ExtensionDigests {
    ...
    /// Hex SHA-256 of the entry blob when one exists (`code_plugin`).
...
pub fn package_extension(project_path: &Path) -> Result<PackageResult, ForgeError> {
    ...
    // code_plugin: hash the entry blob when it exists on disk.
    let mut code_sha256 = None;
    if let Some(entry) = &manifest.entry {
        let entry_path = project_path.join(entry);
        if entry_path.exists() {
            code_sha256 = Some(sha256_hex(&fs::read(&entry_path)?));
